...
circuit.name = 'TBJPNP02CAop'; % File name
circuit.dir = [pwd '\'];  % Sets simulation dir
circuit.theme  = 'boost'; % clean or boost

% Config simulation
circuit.parnamesim={'Vcc','RB','RC','RE'}; % Variables names
circuit.parname={'Vcc','RB','RC','RE'}; % Variables names
circuit.parunit={'V','&Omega;','&Omega;','&Omega;'}; % Variables unit

% parameters input
Vcc=10:5:20; 
% Vz=[2.7 3.3 3.9 4.7 5.6 6.8 8.2 3.0 3.6 4.3 5.1 6.2 7.5 9.1];
Rb = combnres(1,10000,'E24',6); % Ativo direto
% Rb = combnres(1,100,'E24',6); % Saturação
...
% circuit.Xi=CombVec(Vcc,Rb,Rc,Re,Is,Beta,Va); %%
...
[tbjmode,logdata,logfiles]=gettbjpnp02mode(Xi);
...
% Generate question
quiz.enunciado = 'Para a configuração polarização de emissor, simule no LTspice o ponto de operação (.op) do circuito apresentado na Figura 1 e determine:';
