...
[circuitmode]=getcomb08mode(Xi);

% Mostra estatítica de modos encontrados
Tmode = table(sum(circuitmode(:)==1),sum(circuitmode(:)==2)) ;
Tmode.Properties.VariableNames = ["SAT","AD"];
disp(Tmode)

indx=find(circuitmode==2); % 4 -> ativo direto
circuit.Xi=Xi(:,indx);
...
q=q+1;
quiz.question{q}.str='Qual o valor da tensão em Rs?';
quiz.question{q}.units={'V'};
...
q=q+1;
quiz.question{q}.str='Qual o valor da corrente em Rs?';
quiz.question{q}.units={'A'};
quiz.question{q}.options={'irs'};
quiz.question{q}.vartype={'op'}; % meas 
quiz.question{q}.optscore=[100]; % Score per option
quiz.question{q}.opttol=[10]; % tolerance in percentage %
quiz.question{q}.type='NUMERICAL';
