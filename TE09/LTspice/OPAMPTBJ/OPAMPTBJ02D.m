...
circuit.name = 'OPAMPTBJ02D'; % File name
...
circuit.funcstr  = {'OPAMPTBJ02Dfunc1(parvalues)','OPAMPTBJ01Dfunc2(parvalues)'}; % Array of strings evalstr
...
circuit.Xi=CombVec(Vs,Vi,R1,RL,Is,nf); %%
...
circuit.model.parnamesim={'IS','N'};
circuit.model.parname={'IS','N'};
circuit.model.parunit={'A',''};

% circuit.Xm=CombVec(Is,Beta,Va); %%
circuit.model.name='D2';
...
q=q+1;
quiz.question{q}.str='Qual o valor da corrente em D2?';
quiz.question{q}.units={'A'};
quiz.question{q}.options={'id2'}; % Only lowcase
quiz.question{q}.vartype={'op'}; % meas 
quiz.question{q}.optscore=[100]; % Score per option
quiz.question{q}.opttol=[5]; % tolerance in percentage %
quiz.question{q}.type='NUMERICAL';
...
q=q+1;
quiz.question{q}.str='Qual o valor da tensão térmica \( V_T \) ?';
quiz.question{q}.units={'V'};
