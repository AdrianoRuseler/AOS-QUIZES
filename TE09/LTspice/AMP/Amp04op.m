...
circuit.name = 'Amp04op'; % File name
circuit.dir = [pwd '\'];  % Sets simulation dir
circuit.theme  = 'boost'; % clean or boost

% Config simulation
circuit.parnamesim={'Vs','V1','V2','R1','R2','RL'}; % Variables names
circuit.parname={'Vs','V1','V2','R1','R2','RL'}; % Variables names
...
quiz.eqlist{1}='\( \left( 1+\dfrac{R_2}{R_1} \right )(V_1 - V_2)\)';
quiz.eqlist{2}='\( -\dfrac{R_2}{R_1}(V_1 - V_2)\)';
quiz.eqlist{3}='\( \left( 1+\dfrac{R_2}{R_1} \right )(V_1 + V_2)\)';
quiz.eqlist{4}='\( \dfrac{V_1}{R_1}-\dfrac{R_2V_2}{R_1(R_1+R_2)}\)';
quiz.eqlist{5}='\( \dfrac{V_1}{R_1+R_2}\)';
quiz.eqlist{6}='\( \dfrac{V_2}{R_1+R_2}\)';
quiz.eqlist{7}='\( \dfrac{V_1}{R_1} + \dfrac{V_2}{R_2}\)';
...
% q=q+1;
% quiz.question{q}.str='Qual o valor da tensão na entrada inversora do AmpOp?';
% quiz.question{q}.units={'V'};
...
% 
% q=q+1;
% quiz.question{q}.str='Qual o valor da tensão na entrada não-inversora do AmpOp?';
% quiz.question{q}.units={'V'};
...
% q=q+1;
% quiz.question{q}.str='b) Qual o valor do ganho de tensão Vo/Vi?';
% quiz.question{q}.units={'V/V'};
...
% quiz.question{q}.type='NUMERICAL';
q=q+1;
quiz.question{q}.str='Qual equação calcula o valor da corrente em R1?';
quiz.question{q}.units={'V/V'};
quiz.question{q}.options=quiz.eqnum; % Only lowcase
quiz.question{q}.vartype=quiz.vartype; % meas 
...
q=q+1;
quiz.question{q}.str='Qual equação calcula o valor da corrente em R4?';
quiz.question{q}.units={'V/V'};
quiz.question{q}.options=quiz.eqnum; % Only lowcase
quiz.question{q}.vartype=quiz.vartype; % meas 
