...
circuit.name = 'Amp03op'; % File name
...
quiz.eqlist={'\( \left( 1+\dfrac{R_2}{R_1} \right )(V_1 - V_2)\)','\( -\dfrac{R_2}{R_1}(V_1 - V_2)\)','\( \left( 1+\dfrac{R_2}{R_1} \right )(V_1 + V_2)\)','\( -\dfrac{R_2}{R_1}(V_1 + V_2)\)','\( \dfrac{V_1}{R_1}\)','\( \dfrac{V_2}{R_1}\)','\( \dfrac{V_1}{R_1} + \dfrac{V_2}{R_1}\)'};
...
q=q+1;
quiz.question{q}.str='Qual equação calcula o valor da corrente em R3?';
quiz.question{q}.units={'V/V'};
quiz.question{q}.options=quiz.eqnum; % Only lowcase
quiz.question{q}.vartype=quiz.vartype; % meas 
...
% q=q+1;
% quiz.question{q}.str='f) Qual o valor da tensão na entrada inversora do AmpOp?';
% quiz.question{q}.units={'V'};
