function [tbjmode,logdata,logfiles]=gettbjnpn02mode(Xi)

% system(['LTspice -Run -b -ascii -netlist TBJ02CAop.asc']); % Cria o arquivo .net
% winopen([circuit.dir circuit.name 'op.net'])

% X0=CombVec(Vcc,Rb,Rc,Re,Is,Beta,Va); %%
netlines{1}='* E:\Dropbox\UTFPR\ET74C\MATLAB\Q04\TBJ02CAop.asc';
...
netlines{10}='.op';
netlines{11}='.model TBJ NPN(IS=10F BF=50 VAF=200)';
